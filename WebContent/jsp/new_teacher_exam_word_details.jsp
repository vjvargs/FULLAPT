	...
	<div class="container">
			...
			<tbody>
				...
				<c:forEach items="${videos }" var="item" varStatus="counter">
					<tr>
						...
						<td><a
							href="${pageContext.request.contextPath }/teacher?method=deleteWordVideo&vid=${item.vid }&lid=<% out.print(l.getLid());%>&eid=<% out.print(e.getEid());%>&wid=<% out.print(w.getFid());%>">x</a></td>
					</tr>
				</c:forEach>
			</tbody>
		</table>
		...
		<div class="wells">
			<form class="form-horizontal" name="l_file"
				action="${pageContext.request.contextPath }/teacher?method=wordAddVideo&lid=<% out.print(l.getLid());%>&eid=<% out.print(e.getEid());%>&wid=<% out.print(w.getFid());%>"
